...
from frontends.qt.theme import C, apply_theme, apply_theme_with_qss, current_theme_name, current_theme, get_theme, on_theme_changed, generate_qss
...
# ── Styles réutilisés ─────────────────────────────────────────────────────
def _search_btn_ss() -> str:
    t = current_theme()
    return (f"QPushButton{{background:transparent;border:none;border-radius:13px}}"
            f"QPushButton:hover{{background:{t.card_bg}}}")

_SEARCH_BTN_SS = _search_btn_ss()
def _close_search_ss() -> str:
    t = current_theme()
    return (f"QPushButton{{background:transparent;color:{t.muted};border:none;font-size:16px}}"
            f"QPushButton:hover{{color:{t.text_secondary}}}")

_CLOSE_SEARCH_SS = _close_search_ss()
def _win_btn_ss() -> str:
    t = current_theme()
    return (f'QPushButton{{background:{t.card_bg};color:{t.text_secondary};border:none;'
            f'border-radius:13px;font-size:12px}}'
            f'QPushButton:hover{{background:{t.border_light};color:white}}')

_WIN_BTN_SS = _win_btn_ss()
def _close_btn_ss() -> str:
    t = current_theme()
    return (f'QPushButton{{background:{t.card_bg};color:{t.text_secondary};border:none;'
            f'border-radius:13px;font-size:14px}}'
            f'QPushButton:hover{{background:rgba(220,38,38,0.85);color:white}}')

_CLOSE_BTN_SS = _close_btn_ss()
def _new_sess_ss() -> str:
    t = current_theme()
    return (f'QPushButton{{background:rgba(124,58,237,0.18);color:#a78bfa;'
            f'border:1px solid rgba(124,58,237,0.3);border-radius:7px;'
            f'padding:0 10px;font-size:12px;font-weight:700}}'
            f'QPushButton:hover{{background:rgba(124,58,237,0.35);color:white}}')

_NEW_SESS_SS = _new_sess_ss()
def _model_menu_ss() -> str:
    """Generate the model-menu stylesheet from the current theme."""
    t = current_theme()
    return (f"QMenu{{background:{t.panel};border:1px solid {t.border};"
            f"padding:4px 0}}QMenu::item{{color:{t.text};padding:6px 20px 6px 12px;"
            f"font-size:12px}}QMenu::item:selected{{background:rgba(63,63,70,0.6)}}")

def _search_input_ss() -> str:
    """Generate the search-input stylesheet from the current theme."""
    t = current_theme()
    return (f"QLineEdit{{background:rgba(32,32,38,0.9);border:1px solid {t.border};"
            f"border-radius:13px;color:{t.text};font-size:13px;padding:0 10px}}"
            f"QLineEdit::placeholder{{color:{t.muted}}}")


class ChatPanel(QWidget):
    """Frameless chat window — thin orchestrator with UI superpowers.

    By default the window is a normal frameless window (appears in taskbar,
    focusable via Alt+Tab).  Pass overlay=True to restore the old always-on-top
    overlay behaviour (WindowStaysOnTopHint + Tool).
    """

    def __init__(self, agent, overlay: bool = False):
        super().__init__()
        self.agent = agent
        ...
        # Normal window mode: frameless but visible in taskbar & Alt+Tab
        # --overlay flag restores the old always-on-top + Tool behaviour
        if overlay:
        ...
        # Circuit breaker state change banner
        self._cb_banner: Optional[QLabel] = None
        self._cb_countdown_timer: Optional[QTimer] = None
        self._subscribe_circuit_breaker_events()
    ...
    def _subscribe_circuit_breaker_events(self):
        """Subscribe to circuit_breaker.state_change events from the event bus."""
        try:
            from agentmain.event_bus import subscribe as eb_subscribe
            eb_subscribe("circuit_breaker.state_change", self._on_circuit_breaker_change)
        except Exception:
            logger.debug("Circuit breaker event subscription skipped", exc_info=True)

    def _on_circuit_breaker_change(self, data: dict):
        """Handle circuit breaker state change events (called from any thread)."""
        try:
            new_state = data.get("new_state", "")
            provider = data.get("provider", "unknown")
            if new_state == "open":
                # Schedule on the Qt thread
                QTimer.singleShot(0, lambda: self._show_circuit_breaker_banner(provider, data))
            elif new_state == "closed":
                QTimer.singleShot(0, self._hide_circuit_breaker_banner)
        except Exception:
            logger.debug("Circuit breaker event handling failed", exc_info=True)

    def _show_circuit_breaker_banner(self, provider: str, data: dict):
        """Show a banner indicating the circuit breaker is open."""
        try:
            from i18n import t as _t
        except ImportError:
            _t = lambda k, *a, **kw: k

        if self._cb_banner is None:
            self._cb_banner = QLabel()
            self._cb_banner.setAlignment(Qt.AlignCenter)
            self._cb_banner.setStyleSheet(
                "QLabel { background: rgba(239,68,68,0.85); color: white;"
                " padding: 4px 12px; font-size: 12px; font-weight: 600;"
                " border-radius: 4px; }"
            )
            # Insert banner at the top of the chat page
            self._chat_page.msg_layout.insertWidget(0, self._cb_banner)

        self._cb_banner.setText(_t("circuit_breaker.open_banner", provider))
        self._cb_banner.show()

        # Start countdown timer
        if self._cb_countdown_timer is not None:
            self._cb_countdown_timer.stop()
        self._cb_countdown_start = time.time()
        self._cb_recovery_timeout = 60.0  # default
        self._cb_countdown_timer = QTimer(self)
        self._cb_countdown_timer.timeout.connect(
            lambda: self._update_cb_countdown(provider)
        )
        self._cb_countdown_timer.start(1000)

    def _update_cb_countdown(self, provider: str):
        """Update the circuit breaker banner countdown."""
        try:
            from i18n import t as _t
        except ImportError:
            _t = lambda k, *a, **kw: k
        ...
        if self._cb_banner is None:
            return
        elapsed = time.time() - getattr(self, '_cb_countdown_start', time.time())
        remaining = max(0, getattr(self, '_cb_recovery_timeout', 60.0) - elapsed)
        if remaining <= 0:
            self._hide_circuit_breaker_banner()
            return
        self._cb_banner.setText(
            _t("circuit_breaker.open_banner_countdown", provider, remaining)
        )

    def _hide_circuit_breaker_banner(self):
        """Hide the circuit breaker banner when the circuit closes."""
        if self._cb_countdown_timer is not None:
            self._cb_countdown_timer.stop()
            self._cb_countdown_timer = None
        if self._cb_banner is not None:
    ...
    def paintEvent(self, _):
        ...
        # Parse theme bg color for gradient
        bg = th.bg.lstrip('#') if th.bg.startswith('#') else '0e0e12'
        r0, g0, b0 = int(bg[0:2], 16), int(bg[2:4], 16), int(bg[4:6], 16)
        grad.setColorAt(0.0, QColor(r0, g0, b0, 228)); grad.setColorAt(1.0, QColor(r0//2, g0//2, b0//2, 242))
    ...
    def _build_ui(self):
        ...
        self._onboarding_page = None  # Created on demand
    ...
    def show_onboarding(self):
        ...
        self._onboarding_page = OnboardingWizard()
        self._stack.insertWidget(0, self._onboarding_page)
        self._stack.setCurrentIndex(0)
        self._onboarding_page.onboarding_complete.connect(self._on_onboarding_complete)

    def _on_onboarding_complete(self, provider: str, api_key: str):
        """Handle onboarding completion — switch to chat page."""
        # Remove onboarding page from stack
        if self._onboarding_page is not None:
            self._stack.removeWidget(self._onboarding_page)
            self._onboarding_page.deleteLater()
            self._onboarding_page = None
        self._switch_tab(0)  # Switch to chat
        self._chat_page.add_system_notice(
            t("onboarding.complete_notice", f"Configuration terminée avec {provider}. Bienvenue !", provider)
        )
    ...
    def _build_titlebar(self):
        bar = QWidget()
        # Titlebar height based on font metrics for HiDPI
        fm = bar.fontMetrics()
        bar.setFixedHeight(max(48, fm.height() * 3))
        ...
        sb.setIconSize(QSize(16,16)); sb.setFixedSize(btn_sz, btn_sz)
        ...
        self._search_input.setPlaceholderText(t("qt.search_placeholder"))
        ...
        cs = QPushButton("×"); cs.setFixedSize(btn_sz, btn_sz); cs.setCursor(QCursor(Qt.PointingHandCursor))
        ...
        btn_sz = max(26, self.fontMetrics().height() + 10)
        for txt, slot in [("─", self.hide), ("□", self._toggle_maximize)]:
            b = QPushButton(txt); b.setFixedSize(btn_sz, btn_sz); b.setCursor(QCursor(Qt.PointingHandCursor))
            b.setStyleSheet(_WIN_BTN_SS); b.clicked.connect(slot); ly.addWidget(b)
        self._maxi_btn = ly.itemAt(ly.count()-1).widget()

        # Close
        cl = QPushButton("×"); cl.setFixedSize(btn_sz, btn_sz); cl.setCursor(QCursor(Qt.PointingHandCursor))
    ...
    def _show_search(self):
        self._search_visible = True; self._search_btn.setFixedSize(0,0)  # Hide search btn
        self._search_widget.show(); self._search_input.setFocus(); self._search_input.selectAll()

    def _hide_search(self):
        self._search_visible = False
        # Restore search button size using font metrics
        fm = self.fontMetrics()
        self._search_btn.setFixedSize(max(26, fm.height() + 10), max(26, fm.height() + 10))
    ...
    def _build_statusbar(self):
        bar = QWidget()
        fm = bar.fontMetrics()
        bar.setFixedHeight(max(24, fm.height() + 12))
    ...
    def _show_model_menu(self, _):
        ...
        for i, client in enumerate(self.agent.llmclients):
            try: name = client.name or t("qt.model_unknown")
            except Exception: name = t("qt.model_unknown")
    ...
    def _build_tabbar(self):
        bar = QWidget()
        fm = bar.fontMetrics()
        bar.setFixedHeight(max(40, fm.height() * 2 + 10))
        bar.setStyleSheet(f"background:{current_theme().panel};")
        ly = QHBoxLayout(bar); ly.setContentsMargins(12,5,12,5); ly.setSpacing(4)
        self._tabs = []
        for svg, txt_key in [(_SVG_CHAT,"qt.tab_chat"),(_SVG_CLOCK,"qt.tab_history"),(_SVG_BOOK,"qt.tab_sop"),(_SVG_GEAR,"qt.tab_settings")]:
            btn = _TabButton(t(txt_key)); btn.setIcon(_svg_icon(t(txt_key), svg, "#b0b0b8")); btn.setIconSize(QSize(14,14))
            btn.clicked.connect(lambda _, idx=len(self._tabs): self._switch_tab(idx))
            ly.addWidget(btn); self._tabs.append(btn)
        ly.addStretch()
        nb = QPushButton(t("qt.new_conversation")); nb.setIcon(_svg_icon("plus",_SVG_PLUS,"#a78bfa"))
        nb.setIconSize(QSize(12,12)); nb.setFixedHeight(max(27, fm.height() + 8)); nb.setStyleSheet(_NEW_SESS_SS)
    ...
    def _shell_confirm_dialog(self, code, code_type):
        ...
        result = ApprovalDialog.ask(
            title=t("qt.shell_confirm_title", code_type),
            description=t("qt.shell_confirm_desc"),
    ...
    def _do_toggle_theme(self):
        ...
        if current == "dark":
            ...
            new_theme = "dark"
        apply_theme_with_qss(QApplication.instance(), new_theme)
    ...
    def _on_stream_done(self, final_text, is_interrupted=False):
        ...
        if self._chat_page.streaming_row:
            ...
            # Mark interrupted messages with distinct visual style
            if is_interrupted:
                self._chat_page.streaming_row.set_interrupted_style()
        ...
        if is_interrupted:
            # Add to history with is_interrupted flag so it's visible in UI
            # but excluded from LLM context
            self._session_mgr.add_message("assistant", final_text, is_interrupted=True)
            ...
            # Task 9.1.3: Desktop notification when app is not in foreground
            self._notify_if_background("GenericAgent", t("qt.notify_response_done"))
    ...
    def _handle_send(self, text, files):
        ...
        prompt = text or t("qt.analyze_files")
    ...
    def _do_switch_to(self, idx):
        ...
        self._chat_page.add_system_notice(t("qt.switched_model", name))
    ...
    def _do_trigger_auto(self):
        self.inject_message(t("qt.auto_trigger"))

    def _do_reset_prompt(self):
        try: self.agent.llmclient.last_tools = ""
        except Exception: logger.debug("Failed to reset prompt tools", exc_info=True)
    ...
    def _model_name(self):
        ...
        except Exception: return t("qt.model_unknown")
    ...
    # ── Desktop notifications (Task 9.1.3) ────────────────────────────────

    def _notify_if_background(self, title: str, message: str) -> None:
        """Show a desktop notification when the app is not in the foreground.

        Uses QSystemTrayIcon.showMessage() for native OS notifications.
        Only notifies when the window is minimized or not the active window,
        to avoid distracting the user when they are already looking at the app.
        ...
        """
        try:
            # Only notify if the app is not the active/foreground window
            app = QApplication.instance()
            if app is None:
                return

            is_minimized = self.isMinimized() if hasattr(self, 'isMinimized') else False
            active_window = app.activeWindow()
            is_foreground = active_window is not None and self.isAncestorOf(active_window)

            if is_minimized or not is_foreground:
                # Find the tray icon to send the notification
                tray = self._find_tray_icon()
                if tray is not None:
    ...
    def _find_tray_icon(self) -> Optional[QSystemTrayIcon]:
        """Find the system tray icon instance from the main() scope.

        Walks up the object hierarchy to find a TrayManager or
        QSystemTrayIcon. Returns None if not found.
        """
        try:
            # The tray is typically created in main() — try to find it
            app = QApplication.instance()
            if app is None:
                return None
            # Check for TrayManager's internal tray icon
            for obj in app.children():
                if isinstance(obj, QSystemTrayIcon):
                    return obj
            # Try the TrayManager class which holds a _tray attribute
            from frontends.qt.tray_icon import TrayManager
            for obj in app.children():
                if isinstance(obj, TrayManager) and hasattr(obj, '_tray'):
                    return obj._tray
        except Exception:
            pass
        return None
    ...
    def _check_for_updates(self):
        ...
        try:
            ...
            def on_update(info):
                if info:
                    # Schedule on the Qt thread
                    QTimer.singleShot(0, lambda: self.show_update_notification(info))
            check_and_notify_async(on_update)
        except Exception:
            logger.debug("Auto-update check failed at startup", exc_info=True)

    def show_update_notification(self, update_info: dict):
        """Show an update notification banner at the top of the chat panel.

        Displays: "Nouvelle version v{version} disponible — [Mettre à jour] [Plus tard]"
        "Mettre à jour" triggers the auto-update process.
        "Plus tard" dismisses the banner.
        """
        version = update_info.get("version", "unknown")

        # If a banner already exists, remove it
        ...
        """)
        bly = QHBoxLayout(banner)
        bly.setContentsMargins(12, 6, 12, 6)
        bly.setSpacing(8)

        # Icon + text
        info_label = QLabel(
            f"🚀 {t('qt.update_banner_text', f'Nouvelle version v{version} disponible', version)}"
        )
        info_label.setStyleSheet(
            f"color: {th.green}; font-size: 12px; font-weight: 600; background: transparent;"
        )
        bly.addWidget(info_label)

        bly.addStretch()

        # "Mettre à jour" button
        update_btn = QPushButton(t("qt.update_action", "Mettre à jour"))
        update_btn.setCursor(QCursor(Qt.PointingHandCursor))
        update_btn.setStyleSheet(f"""
            QPushButton {{
                background: {th.green}; color: white;
                border: none; border-radius: 4px;
                padding: 4px 12px; font-size: 11px; font-weight: 600;
            }}
            QPushButton:hover {{ background: rgba(34,197,94,0.8); }}
        """)
        update_btn.clicked.connect(lambda: self._do_update(update_info))
        bly.addWidget(update_btn)

        # "Plus tard" button
        dismiss_btn = QPushButton(t("qt.update_dismiss", "Plus tard"))
        ...
        bly.addWidget(dismiss_btn)

        # Insert at the top of the chat page message layout
        self._chat_page.msg_layout.insertWidget(0, banner)
        self._update_banner = banner

        # Also notify settings page
        try:
            if hasattr(self._settings_page, 'set_update_available'):
                self._settings_page.set_update_available(update_info)
        except Exception:
            logger.debug("Failed to set update availability in settings page", exc_info=True)
    ...
    def _do_update(self, update_info: dict):
        ...
        try:
            from agentmain.auto_update import download_update, apply_update
            self._chat_page.add_system_notice(
                t("qt.update_downloading", "Téléchargement de la mise à jour...")
            )
            def download_thread():
                path = download_update(update_info)
                if path:
                    QTimer.singleShot(0, lambda: self._chat_page.add_system_notice(
                        t("qt.update_applying", "Application de la mise à jour...")
                    ))
                    import time; time.sleep(1)
                    apply_update(path)
                else:
                    QTimer.singleShot(0, lambda: self._chat_page.add_system_notice(
                        t("qt.update_failed", "Échec du téléchargement de la mise à jour.")
                    ))
            import threading
            threading.Thread(target=download_thread, daemon=True).start()
        except Exception:
            logger.debug("Auto-update process failed", exc_info=True)
            self._chat_page.add_system_notice(
                t("qt.update_failed", "Échec de la mise à jour.")
            )
...
# ══════════════════════════════════════════════════════════════════════
def main():
    # --overlay CLI flag: restore always-on-top overlay behaviour
    _overlay_mode = "--overlay" in sys.argv
    QApplication.setHighDpiScaleFactorRoundingPolicy(Qt.HighDpiScaleFactorRoundingPolicy.PassThrough)
    app = QApplication(sys.argv)
    app.setQuitOnLastWindowClosed(False); app.setApplicationName("GenericAgent")
    app.setAttribute(Qt.AA_EnableHighDpiScaling)
    # Apply theme with QSS at startup
    apply_theme_with_qss(app, "dark")
    font = QFont()
    ...
    if agent.llmclient is None:
        QMessageBox.critical(None, t("qt.llm_not_configured_title"),
            t("qt.llm_not_configured_msg"))
    else:
        import threading; threading.Thread(target=agent.run, daemon=True).start()

    panel = ChatPanel(agent, overlay=_overlay_mode)
    # Show onboarding wizard on first launch if no credentials exist
    if not has_credentials():
        panel.show_onboarding()
    ...
    print("[GenericAgent]", t("qt.startup_success"))
