...
_logger = logging.getLogger("langfuse_tracing")
...
try:
    ...
    _lf = None
    _logger.debug("Langfuse initialization skipped")
...
if _lf:
    ...
    def _patched_log(label, content):
        ...
        except Exception: _logger.debug("Langfuse patched_log error", exc_info=True)
    ...
    def _wrap_parser(orig):
        def wrapped(resp_lines, *a, **kw):
            ...
            except Exception: _logger.debug("Langfuse usage extraction failed", exc_info=True)
            return ret
        return wrapped
    ...
    def _patched_before(self, tool_name, args, response):
        ...
        except Exception: _logger.debug("Langfuse tool_before trace error", exc_info=True)
    ...
    def _patched_after(self, tool_name, args, response, ret):
        ...
        except Exception: _logger.debug("Langfuse tool_after trace error", exc_info=True)
    ...
    def _patched_loop(client, system_prompt, user_input, handler, tools_schema, *a, **kw):
        try: cm = _lf.start_as_current_observation(name='agent.task', as_type='agent', input={'user_input': user_input})
            ...
            _logger.debug("Langfuse agent trace start failed", exc_info=True)
            cm = None
        if cm is None:
        ...
        with cm as sp:
            ...
            except Exception: _logger.debug("Langfuse agent trace update failed", exc_info=True)
        try: _lf.flush()
        except Exception: _logger.debug("Langfuse flush failed", exc_info=True)
        return ret
    ...
    for _m in list(sys.modules.values()):
        if _m and getattr(_m, 'agent_runner_loop', None) is _orig_loop:
            ...
            except Exception: _logger.debug("Langfuse module patching failed for %s", getattr(_m, '__name__', '?'))
